...
<img src="xorpd_0x3c_animation.gif"/>
 ...
 It's worth pointing out that the two reflection operations used to generate the curve can be performed in any order
without changing the result (technically, these two operations generate the [Klein four-group](https://en.wikipedia.org/wiki/Klein_four-group), which is commutative). Hence,
when generating the Hilbert curve, we only need to keep track of the number of reflection of each type. As reflections of the same type
cancel each other in pairs, it is in fact enough to keep track of the parity of the number of operations. These properties make the
code for this snippet especially simple. Unfortunately, the Klein four-group is the only group generated by reflections that is fully
commutative, which means that the code for generating curves with different symmetries (e.g. the Hilbert curve in 3D using a base-8 represetation) is much
more complicated as it will need to keep track of the order of operations. Indeed, when one day we encounter a civilization that constructed computers
based on the ternary logic of negative, zero and positive currents, their version of snippet 0x3c will be much more complicated!

<p align="center">
 <img src="xorpd_0x3c_sierpinski.gif"/>
</p>
